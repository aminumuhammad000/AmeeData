...
export class AdminCareController {
    ...
    static async getStats(req, res) {
        try {
            const [totalRequests, pendingRequests, acceptedRequests, totalCircleMembers, totalCareVolumeResult, totalCareBalanceResult] = await Promise.all([
     ...
     */
    static async getCircleMemberships(req, res) {
        try {
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            const skip = (page - 1) * limit;
