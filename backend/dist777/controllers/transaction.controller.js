        ...
        try {
            ...
            const enriched = await Promise.all(transactions.map(async (txn) => {
                ...
                if (!plan_name && txn.plan_id) {
                    ...
                    const airtimePlan = await AirtimePlan.findById(planId).lean();
                    if (airtimePlan)
                        plan_name = airtimePlan.name;
                }
                // 3. Fall back to metadata or description
                if (!plan_name && txn.metadata?.plan_name)
                    plan_name = txn.metadata.plan_name;
                if (!plan_name && txn.metadata?.planName)
                    plan_name = txn.metadata.planName;
