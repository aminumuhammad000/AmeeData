...
export class ReferralService {
   ...
   */
  static async processReferralReward(userId: string | Types.ObjectId, amount: number) {
    try {
      ...
      if (settings.referrer_bonus_amount > 0) {
        await this.applyBonus(
          referrerId, 
          settings.referrer_bonus_amount, 
          `Referral bonus for inviting ${user.first_name} ${user.last_name}`,
          user._id
        );
      }
  ...
  private static async applyBonus(userId: string | Types.ObjectId, amount: number, description: string, relatedUserId?: string | Types.ObjectId) {
