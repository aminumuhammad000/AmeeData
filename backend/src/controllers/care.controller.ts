...
export class CareController {
   ...
   */
  static async getCircle(req: AuthRequest, res: Response) {
    try {
  ...
  static async addMember(req: AuthRequest, res: Response) {
    try {
      ...
      if (phone_number && !targetId) {
        ...
        if (!user) return ApiResponse.error(res, 'User not found in AmeeData community', 404);
        targetId = user._id;
      }

      if (!targetId) return ApiResponse.error(res, 'Member ID or Phone is required', 400);
      if (targetId.toString() === req.user?.id) return ApiResponse.error(res, 'Cannot add yourself to your circle', 400);

      const existing = await CareCircleMember.findOne({ user_id: req.user?.id, member_id: targetId });
      if (existing) return ApiResponse.error(res, 'This person is already in your Care Circle', 400);
  ...
  static async updateMember(req: AuthRequest, res: Response) {
    try {
      ...
      const member = await CareCircleMember.findOneAndUpdate(
  ...
  static async removeMember(req: AuthRequest, res: Response) {
    try {
      ...
      if (!result) return ApiResponse.error(res, 'Member not found', 404);
  ...
  static async getStats(req: AuthRequest, res: Response) {
    try {
      ...
      if (memberRecord && (memberRecord.member_id as any).phone_number) {
        const phone = (memberRecord.member_id as any).phone_number;
   ...
   */
  static async requestCare(req: AuthRequest, res: Response) {
    try {
  ...
  static async respondToRequest(req: AuthRequest, res: Response) {
    try {
      ...
      if (!senderWallet) return ApiResponse.error(res, 'Your wallet was not found', 404);
      ...
      try {
        ...
        await WalletService.debitWallet(provider_id as any, amount, session);

        // 2. Credit Requester (Recipient)
        await WalletService.creditWallet(requesterId as any, amount, false, session);
        ...
        const recipientWallet = await Wallet.findOne({ user_id: requesterId }).session(session as any);
        if (recipientWallet) {
          const provider = await User.findById(provider_id).session(session as any);
          await Transaction.create([{
            user_id: requesterId,
        ...
        NotificationService.sendDirectNotification(requesterId as any, {
          type: 'care_fulfilled',
