...
import { createProvider, deleteProvider, getProviderEnv, getProviders, getSystemSettings, testProviderConnection, testProviderPurchase, updateProvider, updateProviderEnv } from '../api/adminApi';
...
const Providers: React.FC = () => {
  ...
  const { data: settingsData } = useQuery({
    ...
    queryFn: () => getSystemSettings().then((r: any) => r.data?.data)
  });
                            ...
                            <div className="flex flex-wrap gap-1">
                              {(p.supported_services || []).map((s: string) => (
                                <div key={s} className="flex flex-col gap-1">
                                  <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded text-[10px] font-medium uppercase border border-slate-200">{s}</span>
                                  {s.toLowerCase() === 'data' && settingsData?.preferred_data_provider === p.code && (
                                    <span className="px-1.5 py-0.5 bg-violet-100 text-violet-700 rounded-full text-[8px] font-bold uppercase ring-1 ring-violet-200">Active Data</span>
                                  )}
                                  {s.toLowerCase() === 'airtime' && settingsData?.preferred_airtime_provider === p.code && (
                                    <span className="px-1.5 py-0.5 bg-indigo-100 text-indigo-700 rounded-full text-[8px] font-bold uppercase ring-1 ring-indigo-200">Active Airtime</span>
                                  )}
                                </div>
                              ))}
                            </div>
            ...
            <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-8 max-h-[85vh] overflow-y-auto">
                ...
                <form onSubmit={onSubmit} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">Supported Services</label>
                    <div className="grid grid-cols-2 gap-2">
                      {['airtime', 'data', 'cable', 'electricity', 'exampin'].map(service => (
                        <label key={service} className="flex items-center gap-2 p-2 border rounded hover:bg-slate-50 cursor-pointer">
                          <input 
                            type="checkbox" 
                            checked={form.supported_services.includes(service)}
                            onChange={(e) => {
                              const services = e.target.checked 
                                ? [...form.supported_services, service]
                                : form.supported_services.filter(s => s !== service);
                              setForm({ ...form, supported_services: services });
                            }}
                          />
                          <span className="text-xs uppercase font-medium">{service}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {['airtime', 'data', 'cable', 'electricity', 'exampin'].map(service => (
                        <label key={service} className="flex items-center gap-2 p-2 border rounded hover:bg-slate-50 cursor-pointer">
                          <input 
                            type="checkbox" 
                            checked={(editItem.supported_services || []).includes(service)}
                            onChange={(e) => {
                              const current = editItem.supported_services || [];
                              const services = e.target.checked 
                                ? [...current, service]
                                : current.filter((s: string) => s !== service);
                              setEditItem({ ...editItem, supported_services: services });
                            }}
                          />
                          <span className="text-xs uppercase font-medium">{service}</span>
                        </label>
